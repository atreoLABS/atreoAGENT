...
func TestIPLimiter_Burst(t *testing.T) {
	...
	if !l.Allow("2.2.2.2") {
		t.Error("different IP should get its own budget")
	}
}

func TestIPLimiter_Reap(t *testing.T) {
	l := newIPLimiter(5)
	l.Allow("1.1.1.1")
	l.Allow("2.2.2.2")
	// Backdate 1.1.1.1's bucket so it looks idle.
	l.mu.Lock()
