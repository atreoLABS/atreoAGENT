...
1. **ACL identity pubkeys pin member identity.** `handler.go` verifies client provision signatures against `member.IdentityPublic` from the ACL — never from `req.*`. This is the keystone of the zero-knowledge story. Don't bypass.
